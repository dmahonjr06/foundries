import { Block, BlockPermutation, Dimension, Direction, Entity, EquipmentSlot, ItemStack, system, Vector3 } from "@minecraft/server"
...
export const inputMold: import("@minecraft/server").BlockCustomComponent = {
    onPlayerInteract({ block, player, face }) {
        ...
        const currentPart       = block.permutation.getState("foundry:part" as keyof BlockStateSuperset) as string;
        const currentMaterial1  = block.permutation.getState("foundry:material_type1" as keyof BlockStateSuperset) as number;
        const currentMaterial2  = block.permutation.getState("foundry:material_type2" as keyof BlockStateSuperset) as number;
        ...
        // ── Retrieve cast item ───────────────────────────────────────────────
        if (hasCast) {
            if (mainhandId !== undefined && mainhandId !== "") return; // must be empty hand
            ...
            block.setPermutation(
                BlockPermutation.resolve("foundry:casting_table", {
...
function encodeMaterial(materialNumber: number): { type1: number, type2: number } {
    return {
...
export function decodeMaterial(type1: number, type2: number): number {
    return type1 + (type2 * 16);
}

// Called by the faucet once pouring is complete
export function solidifyCastingTable(castingTableBlock: Block, materialNumber: number): void {
    const currentPart = castingTableBlock.permutation.getState("foundry:part" as keyof BlockStateSuperset) as string;
    if (!currentPart || currentPart === "empty") return;

    const { type1, type2 } = encodeMaterial(materialNumber);
    const cardinalDirection = castingTableBlock.permutation.getState("minecraft:cardinal_direction" as keyof BlockStateSuperset) ?? "north";

    system.waitTicks(40).then(() => {
        castingTableBlock.setPermutation(
            BlockPermutation.resolve("foundry:casting_table", {
                "foundry:part":            currentPart,
        ...
        castingTableBlock.dimension
            ...
            .forEach(entity => {
                if (entity.typeId !== "foundry:casting_table") return;
...
export function castingTablePourNextLayer(
    foundryLiquidEntities: Entity[],
    castingTableEntity: Entity,
    castingTableBlock: Block,
    currentFill: number,
    ...
    foundryLiquidEntities.forEach(entity => {
        ...
        for (let i = 16; i >= 1; i--) {
            ...
            if (layerMaterial !== incomingMaterial) break;
...
export function faucetPourIntoCastingTable(block: Block, dimension: Dimension): void {
    setIsPouring(false);
    if (getIsPouring()) {
    ...
    if (!castingTableBlock) return;

    // Get the mold type from the block state
    const moldKey = castingTableBlock.permutation.getState("foundry:part" as keyof BlockStateSuperset) as string;
    ...
    const mat1 = castingTableBlock.permutation.getState("foundry:material_type1" as keyof BlockStateSuperset) as number;
    const mat2 = castingTableBlock.permutation.getState("foundry:material_type2" as keyof BlockStateSuperset) as number;
    if (mat1 !== 0 || mat2 !== 0) {
    ...
    let castingTableEntity: Entity | undefined;
    castingTableEntities.forEach(entity => {
        if (entity.typeId === "foundry:casting_table") castingTableEntity = entity;
    });
    ...
    const currentFill = castingTableEntity.getProperty("casting_table:pour_state") as number;
