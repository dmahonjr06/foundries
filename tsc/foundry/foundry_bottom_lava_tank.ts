...
import { BlockPermutation, EquipmentSlot, ItemStack, Vector3 } from "@minecraft/server";


// Can only put lava in, can't take out yet. (Just for testing)
//  TODO: Make the tank accept a database of bucket types and returns.
export const lava_tank_component: import("@minecraft/server").BlockCustomComponent = {
    onPlayerInteract({block, player}){
        const cardinalDirection = block.permutation.getState("minecraft:cardinal_direction" as keyof BlockStateSuperset) as string;
        const lava_level = block.permutation.getState("foundry:lava_level" as keyof BlockStateSuperset) as number;
        const selectedSlot = player?.selectedSlotIndex as number;
        ...
        if (multiBlockPart !== 0) return; // Only allow interaction with the bottom block of the multi-block structure.
        // Lava Tank full.
        if (lava_level === 12) return player?.onScreenDisplay.setActionBar("Lava Tank is full!"); 

        let newLavaLevel = lava_level;
        let itemToGive: ItemStack | undefined = undefined;
        
        if (itemId !== "minecraft:lava_bucket") return;
...
export const player_interact_with_foundry: import("@minecraft/server").BlockCustomComponent = {
    onPlayerInteract({ block, dimension, player }){
        ...
        const foundryEntityLocation = block.above(1) as Vector3
