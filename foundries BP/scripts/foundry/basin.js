...
export const retrieve_block_from_basin = {
    onPlayerInteract({ block, player }) {
        ...
        if (!blockTypes)
            return;
        if (!blockTypes.itemToGive)
            return;
        // Player takes Block from Basin
        const resourceType1 = block.permutation.getState("basin:material_type1");
        const resourceType2 = block.permutation.getState("basin:material_type2");
        if (resourceType1 === "0" && resourceType2 === "0")
            return;
